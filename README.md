...
funcscope 是一个 **函数级 / 代码片段级执行耗时采集工具**，面向 **ARM 与 x86 平台**，通过在源码中显式插桩的方式，对函数或指定代码区间的执行时间进行低开销度量。

该项目并非通用性能分析器（Profiler），而是一个 **源码级、边界明确、低心智负担的耗时量化工具**，用于补充 perf / bpftrace 在 **精确代码边界与单次调用耗时** 场景下的不足。

funcscope 在函数入口与出口记录时间差，并将结果写入每函数独立的环形数组，实现 **零锁、零系统调用** 的采集路径。  
统计逻辑与业务代码解耦，由独立工具按固定周期（如 1 秒）读取数据并计算平均值、最大值或分布信息，从而避免在热路径中进行原子操作和复杂统计，对原有性能影响可控。

与 perf 的采样模型不同，funcscope 提供的是 **确定性的单次调用耗时数据**，不用于自动定位热点，而用于 **量化优化效果、验证性能回归、分析尾部延迟**。

该工具适用于 **UDP / 网络代理 / 协议栈 / 事件驱动程序** 等性能敏感场景。  
不支持递归函数，不涉及线程调度，仅关注函数或代码片段的执行耗时，可在生产代码中按需启用或关闭。
